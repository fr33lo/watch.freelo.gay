    ...
    (a.modules = {
      ...
      10525: (a, b, c) => {
        ...
        class k {
          ...
          static cacheKey(a, b) {
            return `${a.toLowerCase().trim()}|${b ?? ''}`;
          }
          static async searchAnime(a, b = 5) {
            if (!j._.MAL_CLIENT_ID)
              throw Error('MAL_CLIENT_ID is not configured');
            let { data: c } = await this.client.get('/anime', {
              headers: { 'X-MAL-CLIENT-ID': j._.MAL_CLIENT_ID },
              params: {
                q: a,
                limit: b,
            ...
            return c?.data?.map((a) => a.node) ?? [];
          }
          static async resolveAnimeId(a, b) {
            let c = this.cacheKey(a, b);
            if (this.cache.has(c)) return this.cache.get(c);
            let d = await this.searchAnime(a, 5),
              e = this.pickBestMatch(a, d, b);
            return (this.cache.set(c, e), e);
          }
          static pickBestMatch(a, b, c) {
            let d;
            if (!b?.length) return;
            let e = (a) => (a ?? '').trim().toLowerCase(),
              f = e(a);
            for (let a of b) {
              let b = 0,
                g = e(a.title),
                h = e(a.alternative_titles?.en ?? void 0),
                i = (a.alternative_titles?.synonyms ?? []).map(e);
              if (
                (g === f && (b += 10),
                h && h === f && (b += 8),
                i.includes(f) && (b += 6),
                g.includes(f) && f.length >= 4 && (b += 4),
                h.includes(f) && f.length >= 4 && (b += 3),
                i.some((a) => a.includes(f) && f.length >= 4) && (b += 2),
                c)
              ) {
                let d = a.start_date
                  ? Number(String(a.start_date).slice(0, 4))
                  : void 0;
                d && d === c && (b += 5);
              }
              (!d || b > d.score) && (d = { item: a, score: b });
            }
            return d?.item ?? b[0];
          }
        }
        ...
        async function m({ params: a }) {
          ...
          if (m && j)
            try {
              let a = await h.A.findTvSeries(Number(j)),
                c =
                  a.data?.name ?? a.data?.original_name ?? a.data?.title ?? '',
                d = a.data?.first_air_date
                  ? Number(a.data.first_air_date.slice(0, 4))
                  : void 0;
              if (c) {
                let a = await k.resolveAnimeId(c, d);
                a?.id && (b = a.id);
              }
            } catch {}
          return (
            (c =
              m && b
                ? (0, g.$)({
                    ...
                    malId: b,
                    episode: 1,
                    options: { autoplay: !1, variant: 'sub', fallback: !0 },
                  })
                : (0, g.$)({
                    provider: 'vidsrc',
                    mediaType: m ? 'anime' : 'movie',
                    tmdbId: j,
                    episode: m ? 1 : void 0,
                    options: m ? { autoplay: !1, variant: 'sub' } : void 0,
                  })),
            (0, d.jsx)(e.default, {
              movieId: l,
              mediaType: m ? f.z.ANIME : void 0,
              url: c,
              malId: b,
              tmdbId: j,
            })
        ...
        let H = {
                          ...
                          '[slug]',
                          {
                            children: [
                              ...
                              {},
                              {
                                page: [
                                  () =>
                                    Promise.resolve().then(c.bind(c, 10525)),
                                  'C:\\Users\\Michael\\Downloads\\watch.freelo.gay-dev\\watch.freelo.gay-dev\\src\\app\\watch\\anime\\[slug]\\page.tsx',
                                ],
                              },
                            ],
                          },
          ...
          K = new d.AppPageRouteModule({
            definition: {
              ...
              page: '/watch/anime/[slug]/page',
              pathname: '/watch/anime/[slug]',
              bundlePath: '',
              filename: '',
              appPaths: [],
            },
...
see more here https://nextjs.org/docs/messages/app-static-to-dynamic-error`),
  ...
  b.C(a);
  var c = b.X(0, [873, 892, 885, 428, 207, 301, 715, 487, 472], () =>
    b((b.s = 86830)),
  );
  module.exports = c;
})();
